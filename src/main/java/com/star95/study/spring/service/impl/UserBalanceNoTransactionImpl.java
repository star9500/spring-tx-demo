 ...
 * @date 2019/1/24
 */
@Component("userBalanceNoTransaction")
public class UserBalanceNoTransactionImpl implements UserBalanceService {
    ...
    public void transferAmount(int fromUserId, int toUserId, double amount, boolean needException) {
        ...
        if (needException) {
            throw new RuntimeException("无事务-转账异常");
        }
        userBalanceDao.rechargeAmount(toUserId, amount);
        System.out.println("无事务-转账成功！");
    }
}
